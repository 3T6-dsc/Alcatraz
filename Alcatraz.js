...
async function init() {
  ...
  // Initialisation asynchrone des Giveaways dans la DB
  const hasGiveaways = await db.get('giveaways');
  if (!hasGiveaways) await db.set('giveaways', []);
...
class GiveawayManagerWithOwnDatabase extends GiveawaysManager {
  async getAllGiveaways() { return await db.get('giveaways'); }
  async saveGiveaway(messageId, giveawayData) { await db.push('giveaways', giveawayData); return true; }
  async editGiveaway(messageId, giveawayData) {
    const giveaways = await db.get('giveaways') || [];
    const updated = giveaways.filter((g) => g.messageId !== messageId);
    updated.push(giveawayData);
    await db.set('giveaways', updated);
    return true;
  }
...
client.on('messageCreate', async message => {
  ...
  let messageFetch = await db.get(`guildMessages_${message.guild.id}`);
  if (messageFetch === null) return;

  await db.add(`messages_${message.guild.id}_${message.author.id}`, 1);
  let msgCount = await db.get(`messages_${message.guild.id}_${message.author.id}`);
